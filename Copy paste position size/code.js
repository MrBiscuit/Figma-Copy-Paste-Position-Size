...
if (figma.command == 'pasteSize') {
    ...
    selectedItems.forEach(element => {
        if (w <= 0.1) {
            figma.closePlugin("width must be >= 0.01");
            return;
        }
        if (h <= 0.1) {
            figma.closePlugin("height must be >= 0.01");
            return;
        }
        ...
        if (element.name.includes('advanced') || element.name.includes('live-area')) {
            element.verticalPadding = h / 2;
            element.horizontalPadding = w / 2;
            return;
        }
        element.resize(w, h);
    });
...
if (figma.command == 'pasteWidth') {
    const w = +root.getPluginData("width");
    if (w <= 0.1) {
        figma.closePlugin("Width must be >= 0.01 to paste");
    }
    selectedItems.forEach(element => {
        if (element.height <= 0.1) {
            changeWidthWithZeroHight(element, w);
            return;
        }
        if (element.name.includes('advanced')) {
            if (w - element.width < 0) {
                element.horizontalPadding = element.masterComponent.horizontalPadding;
            }
            element.itemSpacing += w - element.width;
            return;
        }
...
if (figma.command == 'pasteHeight') {
    const h = +root.getPluginData("height");
    if (h <= 0.1) {
        figma.closePlugin("Height must be >= 0.01 to paste");
    }
    selectedItems.forEach(element => {
        if (element.width <= 0.1) {
            changeHeightWithZeroWidth(element, h);
            return;
        }
        if (element.name.includes('advanced') || element.name.includes('live-area')) {
            element.verticalPadding = h / 2;
...
function changeWidthWithZeroHight(node, newWidth) {
...
function changeHeightWithZeroWidth(node, newHeight) {
    let tempFrame = figma.createFrame();
    tempFrame.x = absoluteX(node);
    tempFrame.y = absoluteY(node);
